##############################################################################################################
# Coluna implementation for the Capacitated Lot-sizing Problem with Parallel Machines
#
# Compact formulation:
#
# \min \sum_{i=1}^{NI} \sum{j=1}^{NM} \sum_{t=1}^{NT} pc_{i,j,t} x_{i,j,t} + sc_{i,j,t} y_{i,j,t} 
#    + \sum_{i=1}^{NI} \sum_{t=1}^{NT} hc_{i,t} s_{i,t}
# Subject to
# s_{i,0}, s_{i,NT} = 0
# \sum_{i=1}^{NT} pt_{i,j,t} x_{i,j,t} + st_{i,j,t} y_{i,j,t} \leq cap_{j,t}, \forall j = 1, \dots, NM; t = 1, \dots, NT
# s_{i,t-1} + \sum{j=1}^{NM} x_{i,j,t} = d_{i,t} + s_{i,t}, \forall i = 1, \dots, NI; t = 1, \dots, NT
# x_{i,j,t} \leq M y_{i,j,t}, \forall i = 1, \dots, NI; j = 1, \dots, NM; t = 1, \dots, NT
# y_{i,j,t} \in \{0, 1\}, \forall i = 1, \dots, NI; j = 1, \dots, NM; t = 1, \dots, NT; t = 1, \dots, NT
# x_{i,t}, s{i,t} \geq 0, \forall i = 1, \dots, NI; j = 1, \dots, NM; t = 1, \dots, NT; t = 1, \dots, NT
#
...
inst = Data.readInstance("$appfolder/Instancias/NCLSLT/6i6p2m/NBB01.dat")

model, x, y, s, dec = Model.cg_clsppm(inst, coluna)
