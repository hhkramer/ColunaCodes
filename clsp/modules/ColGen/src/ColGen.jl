...
function cg_clsp(inst::InstanceData)
    ...
    clsp = BlockModel(with_optimizer(Coluna.Optimizer, params = cg_params,
                                 master_factory = with_optimizer(Gurobi.Optimizer),
                                 pricing_factory = with_optimizer(Gurobi.Optimizer)),
                                 bridge_constraints=false)

    # Create variables
    @variable(clsp, x[i=1:I,t=1:inst.numPer] >= 0)	# Producao
    @variable(clsp, y[i=1:I,t=1:inst.numPer], Bin)	# Setup
    @variable(clsp, s[i=1:I,t=1:inst.numPer] >= 0)	# Estoque

    # Create objective function
    @objective(clsp, Min, sum(inst.pc * x[i,t] + inst.sc[i] * y[i,t] + inst.hc[i] * s[i,t] for i=1:I, t=1:inst.numPer))

    # Create capacity constraints
    @constraint(clsp, capConstr[t=1:inst.numPer], sum(inst.pt[i] * x[i,t] + inst.st[i] * y[i,t] for i=1:I) <= inst.cap)

    # Create inventory balance constraints
    @constraint(clsp, iniBalConstr[i=1:I,1], x[i,1] - s[i,1] == inst.dem[i,1])
    @constraint(clsp, balConstr[i=1:I,t=2:inst.numPer], s[i,t-1] + x[i,t] - s[i,t] == inst.dem[i,t])

    # Create setup constraints
    @constraint(clsp, setupConstr[i=1:I,t=1:inst.numPer], x[i,t] - (M * y[i,t]) <= 0)
